...
def explain(client, question: str, cypher: str, rows: list[dict]) -> Answer:
    prompt = (
        ...
        "medical knowledge. If the results include baseline_means/"
        "baseline_stds/deviating_zscores, those are always that specific "
        "athlete's own historical baseline — phrase it as 'above her own "
        "recent average', never as a population norm or a recommended/safe "
        "threshold, since no such external benchmark exists in this data. "
        "If the results are empty, say so plainly rather "
